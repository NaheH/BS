...
(function () {
    ...
    var game = {
        ...
        fire: function (from, col, line, callback) {
            this.wait();
            const audiio = new Audio ('../sound/hitsmtg.mp3');
            audiio.play();
            var self = this;
            var msg = "";
        ...
        renderMiniMap: function () {
            self = this;
            player.fleet.forEach(function(e){
               let clone = e.dom.cloneNode();
               self.miniGrid.appendChild(clone);
            });
        }
    };
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var display = document.querySelector('.game-info');
        var select = document.createElement('select');
        var ops1, ops2, ops3, defaut;
        defaut = document.createElement('option');
        defaut.textContent = "Who starts?";
        ops1 = document.createElement('option');
        ops1.textContent = "Le joueur commence";
        ops1.setAttribute("value","0");
        ops2 = document.createElement('option');
        ops2.textContent = "L'ordinateur commence";
        ops2.setAttribute("value","1");
        ops3 = document.createElement('option');
        ops3.textContent = "Randomize";
        ops3.setAttribute("value","2");
        select.appendChild(defaut);
        select.appendChild(ops1);
        select.appendChild(ops2);
        select.appendChild(ops3);
        display.appendChild(select);
        
        document.querySelector("select").onchange = ()=>{
            var rand;
            if(display.querySelector('select').value === "2"){
                rand = Math.floor(Math.random()*10)%2
            } else {
                rand = display.querySelector('select').value;
            }
            game.start(parseInt(rand));
            game.init();
        };
